...
def parse_args():
    ...
    parser.add_argument("--input_metadata", type=str, required=True, help="Path to metadata.jsonl produced by the preprocessing pipeline.")
...
def main():
    ...
    updated_entries = []
    def resolve_mask_paths(entry):
        candidate_paths = []

        # Primary source: explicit list of mask paths generated by the unconditional
        # segmentation stage.
        for rel_path in entry.get("mask_paths", []) or []:
            mask_path = dataset_root / rel_path
            if mask_path.exists():
                candidate_paths.append(mask_path)

        # Backwards compatibility with the legacy TokenCompose pipeline where masks
        # were stored via attn_list.
        if not candidate_paths:
            for _, rel_path in entry.get("attn_list", []) or []:
                if rel_path is None:
                    continue
                mask_path = dataset_root / rel_path
                if mask_path.exists():
        ...
        # Fallback: scan a sibling directory named "seg/<image_stem>" for mask PNGs.
        if not candidate_paths:
            stem = Path(entry["file_name"]).stem
            fallback_dir = dataset_root / "seg" / stem
            if fallback_dir.exists():
                candidate_paths.extend(sorted(fallback_dir.glob("*.png")))

        return candidate_paths

    for entry in tqdm(entries, desc="Boundary maps"):
        mask_paths = resolve_mask_paths(entry)
